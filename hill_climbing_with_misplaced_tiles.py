...
class node:
    ...
    def generate_sub_space(self, parent, h=None, total_nodes=None):
        ...
        return [s for s in children if s]
    ...
    def hill_climbing_with_misplaced_Tiles(self):
        ...
        queue = []
        start_node = node(start_state, [], '', self.heuristic_tiles(start_state))
        ...
        while (queue):
            ...
            h_val = self.heuristic_tiles(current_node.state)
            next_state = False
            for s in self.generate_sub_space(current_node,self.heuristic_tiles(current_node.state), total_nodes):
                h_val_next = self.heuristic_tiles(s.state)
                if h_val_next <= h_val:
                    next_state = s
                    h_val = h_val_next
            ...
            if next_state:
                ...
                print("\n Hill Climbing with tile displacement terminated after " + str(
...
if __name__ == "__main__":
    ...
    obj1.hill_climbing_with_misplaced_Tiles()
